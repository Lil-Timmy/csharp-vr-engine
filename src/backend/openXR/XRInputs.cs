    ...
    public void UpdateActions(XRInstance _xrInstance, XRSession _xrSession, XRSpace _xrSpace, XRView[] _views, long _predictedDisplayTime)
    {
        ...
        if (_rightIndex != -1)
        {
            ...
            if (rightControllerActive)
            {
                ...
                Array.ForEach(_profile.rightBools , _action => inputBools [_action.input.index] = _action.GetBool (_xrSession));
                Array.ForEach(_profile.rightFloats, _action => inputFloats[_action.input.index] = _action.GetFloat(_xrSession));
                Array.ForEach(_profile.rightVecs  , _action => inputVecs  [_action.input.index] = _action.GetVec2 (_xrSession));
            }
        }
        ...
        if (_leftIndex != -1)
        {
            ...
            if (leftControllerActive)
            {
                ...
                Array.ForEach(_profile.leftBools , _action => inputBools [_action.input.index] = _action.GetBool (_xrSession));
                Array.ForEach(_profile.leftFloats, _action => inputFloats[_action.input.index] = _action.GetFloat(_xrSession));
                Array.ForEach(_profile.leftVecs  , _action => inputVecs  [_action.input.index] = _action.GetVec2 (_xrSession));
            }
        }
    }
}